...
const availableEntities = [
  { id: 'devices', name: 'Devices', description: 'Import device information from Intune' },
  { id: 'users', name: 'Users', description: 'Import user information from Azure AD' },
  { id: 'groups', name: 'Groups', description: 'Import security and distribution groups' }
];
...
const fetchMicrosoftAuthProvider = async () => {
  try {
    ...
    if (response.data && Array.isArray(response.data)) {
      ...
      const microsoftProvider = response.data.find(provider => 
        provider.provider_type === 'microsoft' && provider.enabled
      );
...
const importCredentialsFromAuthProvider = async () => {
  ...
  try {
    ...
    if (microsoftProvider.configs && Array.isArray(microsoftProvider.configs)) {
      ...
      const clientIdConfig = microsoftProvider.configs.find(c => c.key === 'client_id');
      const tenantIdConfig = microsoftProvider.configs.find(c => c.key === 'tenant_id');
      ...
      if (clientIdConfig && tenantIdConfig) {
        ...
        successMessage.value = 'Successfully imported Microsoft Entra credentials';
        ...
        errorMessage.value = 'Microsoft Entra provider is missing required configuration';
      }
    } else {
    ...
    errorMessage.value = error.response?.data?.message || 'Failed to import credentials';
  } finally {
...
const testConnection = async () => {
  ...
  try {
    ...
    errorMessage.value = error.response?.data?.message || 'Failed to test connection';
    connectionStatus.value = 'error';
  } finally {
    isLoading.value = false;
  }
...
const startSync = async () => {
  ...
  try {
    ...
    errorMessage.value = error.response?.data?.message || 'Failed to start sync';
  } finally {
    isLoading.value = false;
  }
};
...
const toggleEntity = (entityId: string) => {
  if (selectedEntities.value.includes(entityId)) {
    selectedEntities.value = selectedEntities.value.filter(id => id !== entityId);
  } else {
...
const getStatusDisplay = (status: string) => {
  switch (status) {
    case 'connected':
      return { text: 'Connected', class: 'bg-green-900/50 text-green-400 border-green-700' };
    case 'disconnected':
      return { text: 'Not Connected', class: 'bg-slate-900/50 text-slate-400 border-slate-700' };
    case 'connecting':
      return { text: 'Connecting...', class: 'bg-blue-900/50 text-blue-400 border-blue-700' };
    case 'error':
      return { text: 'Connection Error', class: 'bg-red-900/50 text-red-400 border-red-700' };
    default:
      return { text: 'Unknown', class: 'bg-slate-900/50 text-slate-400 border-slate-700' };
  }
};
...
<template>
  <div class="flex-1">
    ...
    <div class="flex flex-col gap-4 px-6 py-4 mx-auto w-full max-w-8xl">
      <div class="mb-6">
        <h1 class="text-2xl font-bold text-white">Microsoft Graph Connection</h1>
        <p class="text-slate-400 mt-2">
          Configure and manage the connection to Microsoft Graph API for importing data from Microsoft services
        </p>
      </div>
      ...
      >
        ...
        <div>
          <p>Microsoft Entra authentication provider is configured. You can <button @click="showImportCredentials" class="text-blue-300 underline hover:text-blue-200">import these credentials</button> to use with Microsoft Graph.</p>
        </div>
      </div>
      ...
      <div class="bg-slate-800 border border-slate-700 rounded-lg p-6 mb-4">
        <div class="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
          <div>
            <h2 class="text-xl font-medium text-white mb-2">Connection Status</h2>
          ...
          <div class="flex flex-wrap gap-3">
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37.836-1.372.253-3.187-1.065-2.572z" />
      ...
      <div class="bg-slate-800 border border-slate-700 rounded-lg p-6">
        <h2 class="text-xl font-medium text-white mb-4">Available Data Entities</h2>
        <p class="text-slate-400 mb-4">
          These are the data entities that can be imported from Microsoft Graph API
        </p>
        
        <div class="space-y-4">
          <div v-for="entity in availableEntities" :key="entity.id" 
               class="p-4 bg-slate-700 rounded-lg border border-slate-600">
            <div class="flex items-start">
              <div class="flex-1">
                <h3 class="text-white font-medium">{{ entity.name }}</h3>
                <p class="text-sm text-slate-400 mt-1">{{ entity.description }}</p>
              </div>
              <div class="ml-4">
                <button 
    ...
    >
      <div class="space-y-4">
        ...
        <div>
          ...
          <input 
            v-model="graphConfig.clientSecret"
            type="password"
            class="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-md text-white placeholder-slate-400 focus:outline-none focus:border-blue-500"
        ...
        <div>
          ...
          <div class="bg-slate-700/50 p-3 rounded-md border border-slate-600">
            <ul class="list-disc list-inside text-sm text-slate-300 space-y-1">
              <li v-for="scope in graphConfig.scopes" :key="scope">{{ scope }}</li>
            </ul>
          </div>
          <p class="text-xs text-slate-400 mt-1">
            These permissions must be granted to your app in the Azure portal
          </p>
        </div>
      </div>
    ...
    >
      <div class="space-y-4">
        <p class="text-slate-300">
          Import credentials from your configured Microsoft Entra authentication provider. This will use the same app registration for Microsoft Graph API.
        </p>
        
        <div class="bg-blue-900/20 border border-blue-800/50 rounded-md p-4 text-blue-300">
          <p class="flex items-start gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span>
              To use Microsoft Graph API, your app registration in Microsoft Entra ID must also have the required Graph API permissions added and granted.
            </span>
          </p>
        </div>
        
        <div class="bg-amber-900/20 border border-amber-800/50 rounded-md p-4 text-amber-300">
          <p class="flex items-start gap-2">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <span>
              You will need to provide the client secret again, as it is not stored in a retrievable format.
            </span>
          </p>
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div class="space-y-2">
          <div v-for="entity in availableEntities" :key="entity.id" 
            ...
            />
            <label :for="`sync-${entity.id}`" class="ml-3 block text-white cursor-pointer">
              {{ entity.name }}
            </label>
          </div>
        </div>
        
        <div class="bg-blue-900/20 border border-blue-800/50 rounded-md p-3 text-sm text-blue-300">
          <p class="flex items-start">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span>
              Synchronization will import the latest data from Microsoft services. This may take several minutes depending on the amount of data.
            </span>
          </p>
        </div>
      </div>
