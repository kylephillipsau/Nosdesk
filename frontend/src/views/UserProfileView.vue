...
const fetchUserData = async () => {
  try {
    ...
    if (!route.params.uuid || route.params.uuid === 'new') {
      ...
      canEdit.value = authStore.isAdmin || authStore.user?.role === 'admin';
      canEditRole.value = authStore.isAdmin || authStore.user?.role === 'admin';
...
const saveUser = async () => {
  try {
    ...
    if (isCreationMode.value) {
      ...
      const updatedUser = await userService.updateUser(userProfile.value.uuid, {
        name: editValues.value.name,
        email: editValues.value.email,
        role: editValues.value.role,
        pronouns: editValues.value.pronouns
      ...
      <div class="flex flex-col gap-4 px-6 py-4 mx-auto w-full max-w-8xl">
        ...
        <div v-if="isCreationMode" class="grid grid-cols-1 xl:grid-cols-2 gap-6">
          ...
          <div class="bg-slate-800 rounded-xl border border-slate-700/50 hover:border-slate-600/50 transition-colors">
            ...
            <div class="p-4">
              <div class="flex flex-col gap-4">
                ...
                <!-- Email -->
                <div class="flex flex-col gap-1.5">
                  <h3 class="text-xs font-medium text-slate-400 uppercase tracking-wide">Email *</h3>
                  <div class="bg-slate-700/50 rounded-lg border border-slate-600/30 hover:border-slate-500/50 transition-colors">
                    <input
                ...
                <!-- Role -->
                <div class="flex flex-col gap-1.5">
                  <h3 class="text-xs font-medium text-slate-400 uppercase tracking-wide">Role</h3>
                  <div class="bg-slate-700/50 rounded-lg border border-slate-600/30 hover:border-slate-500/50 transition-colors">
                    <select
                    ...
                    >
                      <option v-for="option in roleOptions" :key="option.value" :value="option.value" class="bg-slate-700">
                        {{ option.label }}
                      </option>
                    </select>
                  </div>
          ...
          <div class="flex justify-end gap-3">
            ...
            <button
              @click="saveUser"
              :disabled="isSaving || !editValues.name || !editValues.email"
        ...
        <div v-else-if="userProfile" class="flex flex-col xl:flex-row gap-6">
          ...
          <div class="flex flex-col gap-4 xl:w-1/2 xl:min-w-0">
            ...
            <UserProfileCard 
              ...
              @success="(message) => {/* TODO: Show success notification */}"
              @error="(message) => {/* TODO: Show error notification */}"
            />
