...
<template>
  <div class="bg-surface rounded-xl border border-default hover:border-strong transition-colors overflow-hidden">
    ...
    <div class="px-4 py-3 bg-surface-hover border-b border-default flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
      <div v-if="showTitle" class="flex items-center gap-3">
        <h2 class="text-lg font-medium text-primary">Your Assigned Tickets</h2>
        <router-link
    ...
    <div v-else-if="tickets.length === 0" class="px-4 py-8 text-center">
      <div class="flex flex-col items-center gap-3">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-tertiary" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
        </svg>
        <div>
          <p class="text-secondary font-medium">No assigned tickets found</p>
          <p class="text-tertiary text-sm mt-1">You're all caught up!</p>
        </div>
      </div>
    </div>
        ...
        <div class="flex gap-4">
          ...
          <div class="flex-1 min-w-0 space-y-2">
            ...
            <div class="flex items-start gap-2">
              <h3 class="text-primary font-medium group-hover:text-blue-400 transition-colors flex-1 leading-snug">
                {{ ticket.title }}
              </h3>
            </div>

            <!-- Metadata row -->
            <div class="flex flex-wrap items-center gap-x-3 gap-y-1.5 text-xs text-secondary">
              ...
              <div class="flex items-center gap-1">
                ...
                <span class="text-tertiary">{{ formatRelativeTime(ticket.modified) }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="flex-shrink-0 flex items-center">
            <svg class="w-5 h-5 text-tertiary group-hover:text-primary group-hover:translate-x-1 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24">
